...
function assertScenario(result, scenario) {
  ...
  if (assertions.location) {
    ...
    const locationMatch =
      propertyRecommendations.some(item => {
  ...
  if (
    ...
    if (
      ...
      if (
        ...
        failures.push(
          `Primary recommendation exceeds budget ${assertions.maxBudget}.`
        );
  ...
  for (
    ...
    if (
      rankedProperties[i].utilityScore >
      rankedProperties[i - 1].utilityScore
    ) {
  ...
  if (assertions.womenOnly) {

    const womenOnlyMatch =
      mobilityRecommendations.some(item => {
  ...
  if (assertions.handleBudgetFailure) {
    ...
    if (
      ...
      failures.push(
        "Budget failure scenario returned no priced alternatives."
      );
    ...
    if (!hasAlternativeContext) {
      ...
      failures.push(
        "Budget failure was detected but no alternative context was provided."
      );
  ...
  if (
    ...
    if (!result?.primary) {
      ...
      if (!primaryExists) {
        ...
        failures.push(
          "Primary recommendation does not exist inside recommendations."
        );
