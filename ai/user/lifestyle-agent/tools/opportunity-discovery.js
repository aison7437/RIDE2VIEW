...
function matchesLocation(item, context, searchText) {
...
function propertyMatchesIntent(
   ...
   * does not contain the neighborhood. Ranking
   * will still prioritize it.
   */
  ...
  for (const neighborhood of requestedNeighborhoods) {
    ...
    if (
      searchText.includes(neighborhood)
    ) {

      if (
        !propertyText.includes(neighborhood)
      ) {
        return false;
      }
