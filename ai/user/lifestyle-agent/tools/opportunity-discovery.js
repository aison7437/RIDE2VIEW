 ...
 * 3. Detect bedrooms
 * 4. Detect budget
 * 5. Detect viewing-time constraints
 * 6. Detect neighborhoods
 * 7. Discover relevant properties
 * 8. Discover mobility when requested
 * 9. Support student / premium / women-only scenarios
 * 10. Preserve the discoverOpportunities() contract
 *
 * Export:
 *
 * module.exports = {
...
function extractBudget(
  ...
  const budgetMatch =
    searchText.match(
      ...
      /(?:ksh|kes|ksh\.|kes\.|budget|for)\s*[, ]?\s*(\d+(?:,\d{3})*|\d+)/i
...
function extractViewingTime(
  ...
  const contextMinuteMatch =
    contextTime.match(
      /(\d+)\s*(?:minutes?|mins?)/i
    );


  if (contextMinuteMatch) {

    const minutes =
      Number(
        contextMinuteMatch[1]
      );
  ...
  const requestHourMatch =
    searchText.match(
      /(?:within|in|under|less than|max(?:imum)?)[^0-9]{0,20}(\d+(?:\.\d+)?)\s*hours?/i
    );
  ...
  if (
    /\bwithin\s+one\s+hour\b/i.test(
      searchText
    ) ||
    /\bin\s+one\s+hour\b/i.test(
      searchText
    )
  ) {
  ...
  const requestMinuteMatch =
    searchText.match(
      /(?:within|in|under|less than|max(?:imum)?)[^0-9]{0,20}(\d+)\s*(?:minutes?|mins?)/i
    );


  if (requestMinuteMatch) {

    const minutes =
      Number(
        requestMinuteMatch[1]
      );
...
function extractNeighborhood(
  ...
  for (
    const neighborhood of neighborhoods
  ) {
...
function formatProperty(
  ...
  return {
    ...
    /*
     * Explicit scenario intelligence
     */
...
async function discoverOpportunities(
  ...
  /*
   * Apply targeted property intent.
   */
  ...
  if (
     ...
     * IMPORTANT:
     *
     * If the user explicitly requested a
     * bedroom count or neighborhood and no
     * result matches, return zero rather than
     * silently returning unrelated properties.
     */
  ...
  propertyResults.sort(
    (a, b) => {
      ...
      if (
        ...
        const aNeighborhood =
          getPropertyText(a).includes(
            intent.neighborhood
          );


        const bNeighborhood =
          getPropertyText(b).includes(
            intent.neighborhood
          );
