...
Detect lifestyle intent

Discover relevant properties

Discover mobility when requested

Support student / premium / women-only scenarios

Preserve the expected discoverOpportunities() contract
...
Depending on the workflow, the request can appear as:
...
This makes discovery resilient even when

some information exists only in context.
...
function buildSearchText(context) {
  ...
  const city =
    normalizeText(context?.location?.city);

  const country =
    normalizeText(context?.location?.country);
...
function detectIntent(context) {
  ...
  const wantsMobility =
    /\btransport\b|\btransportation\b|\bmobility\b|\bget me there\b|\bget there\b|\bdrive\b|\bride\b|\btaxi\b|\bdriver\b/i
      .test(searchText);

  const wantsProperty =
    /\bproperty\b|\bhouse\b|\bhome\b|\bapartment\b|\baccommodation\b|\brent\b|\bhousing\b|\bvilla\b|\bmansion\b/i
      .test(searchText)
    || normalizeText(context?.userGoal) === "property";
...
function matchesLocation(
  ...
  if (itemCity === requestedCity) {
    return true;
  }

  return searchText.includes(
    normalizeText(itemCity)
  );
}
...
function propertyMatchesIntent(
   ...
   *
   * This is now correctly wrapped in backticks.
   */
  ...
  if (
    ...
    Number(property?.property?.bedrooms) !==
      intent.bedrooms
  ) {
    return false;
  }
   ...
   *
   * We do not require it here if the dataset
   * does not contain the neighborhood.
   * Ranking will still prioritize it.
   */
